...
bool Non_Crossing_Biased_Climb_new()
{
...
bool Non_Crossing_Biased_Climb_old()
{
    int upward_preferred;
    int upward_crossing_situation;
    bool result;
...
int alt_sep_test_old()
{
    ...
    {
	need_upward_RA = Non_Crossing_Biased_Climb_old() && Own_Below_Threat();
...
int alt_sep_test_new()
{
    ...
    if (enabled && ((tcas_equipped && intent_not_known) || !tcas_equipped))
    {
	need_upward_RA = Non_Crossing_Biased_Climb_new() && Own_Below_Threat();
	need_downward_RA = Non_Crossing_Biased_Descend() && Own_Above_Threat();
	if (need_upward_RA && need_downward_RA)
        /* unreachable: requires Own_Below_Threat and Own_Above_Threat
           to both be true - that requires Own_Tracked_Alt < Other_Tracked_Alt
...
char *argv[];
{
    ...
    assert(alt_sep_test_new() == alt_sep_test_old());
    exit(0);
}